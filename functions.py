...
#considering geofence is dictionary
'''
	rad:x
	id
	latitude
...
def GetGeoFence(Geofences,SentinelDistance,user_coords):

	activegeofence={}
	for geofence in Geofences.items():

		idofgeofence=geofence[0]
		Latitudes=geofence[1]['latitudes']
		Longitudes=geofence[1]['longitudes']

		typepolygon=geofence[1]['type']
		centroid=geofence[1]['centroid']

		rad=geofence[1]['rad']

		bufferdistance=CalculateBufferDistance(type,Latitudes,Longitudes,rad)

		tempval=GetStatus(type,centroid,Latitudes,Longitudes,bufferdistance,user_coords,rad)

		if(tempval[0]!=1 and tempval[1]<=SentinelDistance): #status ==1

			if idofgeofence not in activegeofence:
				activegeofence[idofgeofence]=[]
			activegeofence[idofgeofence].append(list(tempval))
